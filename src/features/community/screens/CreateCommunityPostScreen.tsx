...
function CreateCommunityPostContent({ navigation }: any) {
  ...
  return (
    <Screen style={{ ...styles.page, backgroundColor: community.surface }}>
      <View style={[styles.topRow, { flexDirection: row, borderBottomColor: community.divider }]}> 
        <Pressable onPress={() => navigation.goBack()} hitSlop={10} style={styles.topIcon}>
          <Ionicons name={isRTL ? 'arrow-forward' : 'arrow-back'} size={27} color={community.text} />
        </Pressable>
      ...
      >
        ...
        >
          <View style={[styles.detailsIcon, { backgroundColor: community.primarySoft }]}> 
            <Ionicons name="options-outline" size={19} color={community.primaryStrong} />
          </View>
          <View style={styles.detailsCopy}>
            <Text style={[styles.detailsTitle, { color: community.text, textAlign: align }]}>
              {ar ? 'إضافة تفاصيل' : 'Add details'}
            </Text>
            <Text style={[styles.detailsSubtitle, { color: community.textMuted, textAlign: align }]}>
              {ar ? 'نوع المنشور، العنوان، المادة والمستوى' : 'Post type, title, subject and level'}
            </Text>
          </View>
          {detailsCount > 0 && (
            <View style={[styles.detailsBadge, { backgroundColor: community.primarySoft }]}> 
              <Text style={[styles.detailsBadgeText, { color: community.primaryStrong }]}>{detailsCount}</Text>
            </View>
          )}
      ...
      <View style={[styles.publishFooter, { backgroundColor: community.surface, borderTopColor: community.divider }]}> 
        <Pressable
          ...
          style={({ pressed }) => [
            ...
            {
              backgroundColor: canPublish ? community.text : community.surfaceRaised,
              opacity: pressed && canPublish ? 0.82 : 1
            }
          ]}
        >
          {busy ? (
            <ActivityIndicator color={community.surface} size="small" />
          ) : (
            <>
              <Ionicons name="paper-plane" size={19} color={canPublish ? community.surface : community.textMuted} />
              <Text style={[styles.publishButtonText, { color: canPublish ? community.surface : community.textMuted }]}>
                {copy.form.publish}
              </Text>
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  bodyInput: { minHeight: 118, fontSize: 17, lineHeight: 25, textAlignVertical: 'top', paddingTop: 4 },
  ...
  detailsIcon: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  detailsCopy: { flex: 1, minWidth: 0 },
  detailsTitle: { fontSize: 14, fontWeight: '800' },
  ...
  detailsBadgeText: { fontSize: 11.5, fontWeight: '800' },
  section: { paddingHorizontal: 15, paddingVertical: 15, gap: 10, borderBottomWidth: StyleSheet.hairlineWidth },
  ...
  publishButtonText: { fontSize: 15, fontWeight: '800' }
});
